...
def compute_next_visit_dates(heb_day, week_flags, agent_user, n=6, horizon_days=180):
    """
    מחשב את N תאריכי הביקור הבאים ללקוח לפי יום הביקור + דגלי השבוע + מחזור הסוכן.
    משתמש ב-get_week_for_agent (מקור האמת של המערכת) כדי שהתצוגה למנהל
    תהיה זהה בדיוק ליומן האמיתי של הסוכן.

    heb_day     — אות עברית של יום הביקור ('א'..'ש')
    week_flags  — dict {1:0/1, 2:0/1, 3:0/1, 4:0/1}
    agent_user  — dict/Row עם name/username (לקביעת מחזור השבוע); None אם אין סוכן
    מחזיר רשימת מחרוזות 'YYYY-MM-DD' (עד n תאריכים).
    """
    from datetime import timedelta
    HEB_TO_PY = {'א': 6, 'ב': 0, 'ג': 1, 'ד': 2, 'ה': 3, 'ו': 4, 'ש': 5}
    target = HEB_TO_PY.get((heb_day or '').strip())
    if target is None or not agent_user:
        return []
    today = now_il().date()
    ...
    while d <= end and len(dates) < n:
        if d.weekday() == target:
            wk = get_week_for_agent(d, agent_user)
            if wk <= 4 and week_flags.get(wk):
...
# ─── לקוחות אבודים (Lost Customers) ──────────────────────────────────────────

def _week_window(ref_date):
    """מחזיר (ראשון, חמישי) של שבוע העבודה (ראשון→חמישי) המכיל את ref_date."""
    from datetime import timedelta
    days_since_sunday = (ref_date.weekday() + 1) % 7  # Sun=0, Mon=1, ... Sat=6
    sunday   = ref_date - timedelta(days=days_since_sunday)
    thursday = sunday + timedelta(days=4)
    return sunday, thursday


def _compute_lost_this_week(db, ref_date):
    """
    מחזיר (lost_map, week_ending_str):
    lost_map = {agent_id: {'agent': row, 'customers': [ {id,name,city,region}, ... ]}}
    לקוח נחשב 'אבוד' אם היה בלו"ז השבוע (assigned_visit_day + דגל השבוע של הסוכן)
    ולא בוקר — כלומר אין לו ביקור בטווח השבוע עם check_out / is_phone / is_unplanned.
    """
    from datetime import timedelta
    sunday, thursday = _week_window(ref_date)
    # אל תספור ימים שעדיין לא הגיעו: הספירה עד היום (ref_date) או עד חמישי — המוקדם.
    # כך בתצוגה חיה באמצע השבוע נספרים רק ימים שעברו; ב-job של חמישי 17:00 נספר כל השבוע.
    cutoff = min(ref_date, thursday)
    week_start_s, week_end_s, cutoff_s = str(sunday), str(thursday), str(cutoff)
    agents = db.execute("SELECT * FROM users WHERE role='agent'").fetchall()
    lost_map = {}
    for agent in agents:
        ...
        if not regions:
            continue
        ph = ','.join(['?'] * len(regions))
        due = {}  # customer_id -> dict (dedupe across the week's days)
        for offset in range(5):  # ראשון..חמישי
            day = sunday + timedelta(days=offset)
            if day > cutoff:  # יום עתידי — הלקוח עדיין לא אמור להיות מבוקר
                break
            heb = HEBREW_DAYS[day.weekday()]
            week_num = get_week_for_agent(day, agent)
            if week_num < 1 or week_num > 6:  # לטבלה יש week_1..week_6 בלבד
                continue
            week_col = f"week_{week_num}"
            try:
                rows = db.execute(
                    f"SELECT id, name, city, region FROM customers "
                    f"WHERE region IN ({ph}) AND assigned_visit_day=? AND {week_col}=1",
                    regions + [heb]
                ).fetchall()
            except Exception:
                continue
            for c in rows:
                due[c['id']] = {'id': c['id'], 'name': c['name'],
                                'city': c['city'] or '', 'region': c['region'] or '',
                                'due_date': str(day)}  # התאריך שבו היה אמור להיות מבוקר
        if not due:
            continue
        lost = []
        for cid, cust in due.items():
            visited = db.execute(
                "SELECT 1 FROM visits WHERE customer_id=? AND visit_date BETWEEN ? AND ? "
                "AND (check_out_time IS NOT NULL OR is_phone=1 OR is_unplanned=1) LIMIT 1",
                (cid, week_start_s, cutoff_s)
            ).fetchone()
            if not visited:
                lost.append(cust)
        if lost:
            lost_map[agent['id']] = {'agent': agent, 'customers': lost}
    return lost_map, week_end_s, cutoff_s


def _prev_weeks_missed_map(db, week_end_s):
    """מונה weeks_missed מהצילום השבועי הקודם (החמישי הקודם) — customer_id -> weeks_missed."""
    prev = db.execute(
        "SELECT customer_id, weeks_missed FROM lost_customer_snapshots "
        "WHERE week_ending = (SELECT MAX(week_ending) FROM lost_customer_snapshots WHERE week_ending < ?)",
        (week_end_s,)
    ).fetchall()
    return {r['customer_id']: r['weeks_missed'] for r in prev}


def _lost_output_from_map(lost_map, prev_map):
    """בונה פלט מקובץ לפי סוכן, עם weeks_missed = רצף (הקודם + 1)."""
    agents_out = []
    for agent_id, data in lost_map.items():
        custs = []
        for c in data['customers']:
            custs.append({
                'id': c['id'], 'name': c['name'], 'city': c['city'], 'region': c['region'],
                'weeks_missed': prev_map.get(c['id'], 0) + 1,
                'due_date': c.get('due_date'),
            })
        custs.sort(key=lambda x: (-x['weeks_missed'], x['name']))
        agents_out.append({'agent_id': agent_id, 'agent_name': data['agent']['name'], 'customers': custs})
    agents_out.sort(key=lambda a: a['agent_name'])
    return agents_out


def compute_lost_customers_weekly(ref_date=None):
    """
    Job מתוזמן (חמישי 17:00): מצלם את הלקוחות האבודים של השבוע ל-lost_customer_snapshots,
    מעדכן מונה שבועות רצופים, ושולח התראה למנהל. idempotent לאותו week_ending.
    """
    db = get_db()
    try:
        ref = ref_date or now_il().date()
        lost_map, week_end_s, _cutoff_s = _compute_lost_this_week(db, ref)
        prev_map = _prev_weeks_missed_map(db, week_end_s)
        now_s = now_il().isoformat()
        db.execute("DELETE FROM lost_customer_snapshots WHERE week_ending=?", (week_end_s,))
        total, repeat = 0, 0
        for agent_id, data in lost_map.items():
            for cust in data['customers']:
                wm = prev_map.get(cust['id'], 0) + 1
                total += 1
                if wm >= 2:
                    repeat += 1
                db.execute(
                    "INSERT INTO lost_customer_snapshots "
                    "(week_ending, customer_id, agent_id, weeks_missed, due_date, created_at) VALUES (?,?,?,?,?,?)",
                    (week_end_s, cust['id'], agent_id, wm, cust.get('due_date'), now_s)
                )
        msg = f"🚨 {total} לקוחות אבודים השבוע (עד {week_end_s})"
        if repeat:
            msg += f" — {repeat} מהם 2+ שבועות ברצף"
        db.execute(
            """INSERT INTO notifications (message, user_name, store_name, action, created_at, is_read)
               VALUES (?, ?, ?, ?, ?, 0)""",
            (msg, "מערכת", "לקוחות אבודים", 'lost_customers', now_s)
        )
        db.commit()
        print(f"[LOST] צילום לקוחות אבודים נשמר — {week_end_s}: {total} אבודים ({repeat} חוזרים)")
        return total, repeat, week_end_s
    finally:
        db.close()
...
async def startup():
    ...
    _scheduler.add_job(compute_lost_customers_weekly, 'cron', day_of_week='thu', hour=17, minute=0, timezone='Asia/Jerusalem')
    _scheduler.start()
...
async def agent_dashboard(request: Request, date: str = None):
    ...
    # בלת"מים של הסוכן להיום
    try:
        db2 = get_db()
        unplanned_visits_raw = db2.execute(
            """SELECT v.id, v.notes, v.customer_id, c.name as store_name, c.city
               FROM visits v JOIN customers c ON v.customer_id=c.id
               WHERE v.user_id=? AND v.visit_date=? AND v.is_unplanned=1
               ORDER BY v.id DESC""",
            (user['id'], str(selected_date))
        ).fetchall()
        db2.close()
        unplanned_visits = [dict(r) for r in unplanned_visits_raw]
    except Exception:
        unplanned_visits = []
...
async def checkout(request: Request):
    ...
    # חובה: הסוכן חייב לעדכן "מה עשיתי בחנות" לפני יציאה (מהשדה עכשיו או ממה שכבר נשמר)
    try:
        _saved_notes = (visit['notes'] or "").strip()
    except Exception:
        _saved_notes = ""
    if not (notes or "").strip() and not _saved_notes:
        db.close()
        return JSONResponse({"error": "עליך לעדכן מה עשית בחנות לפני היציאה", "need_notes": True}, status_code=400)
...
@app.post("/api/unplanned-visit")
async def save_unplanned_visit(request: Request):
    user = get_current_user(request)
    if not user or user['role'] != 'agent':
        return JSONResponse({"error": "לא מורשה"}, status_code=403)
    data = await request.json()
    customer_id = data.get("customer_id")
    notes = data.get("notes", "").strip()
    visit_date = now_il().strftime("%Y-%m-%d")
    if not customer_id or not notes:
        return JSONResponse({"error": "חסרים פרטים"}, status_code=400)
    db = get_db()
    existing = db.execute(
        "SELECT id FROM visits WHERE user_id=? AND customer_id=? AND visit_date=? AND is_unplanned=1",
        (user['id'], customer_id, visit_date)
    ).fetchone()
    if existing:
        ...
        db.execute(
            "INSERT INTO visits (user_id, customer_id, visit_date, notes, is_unplanned) VALUES (?,?,?,?,1)",
            (user['id'], customer_id, visit_date, notes)
        )
    db.commit()
    customer_row = db.execute("SELECT name FROM customers WHERE id=?", (customer_id,)).fetchone()
    store_name = customer_row['name'] if customer_row else f"לקוח #{customer_id}"
    db.close()
    try:
        _send_push_to_managers(
            title=f"⚡ בלת\"ם — {user['name']}",
            body=f"{store_name}: {notes[:80]}",
            url=f"/manager"
        )
    except Exception:
        pass
    return JSONResponse({"status": "ok", "store_name": store_name})
...
async def manager_dashboard(request: Request, date: str = None, agent_id: int = None):
    ...
    for agent in agents:
        ...
        try:
            unplanned_count = db.execute(
                "SELECT COUNT(*) FROM visits WHERE user_id=? AND visit_date=? AND is_unplanned=1",
                (agent['id'], str(selected_date))
            ).fetchone()[0]
        except Exception:
            unplanned_count = 0
...
async def search_customers_agent(request: Request, q: str = ""):
    ...
    try:
        ...
        cust_cols = ("id, name, city, region, visit_day, assigned_visit_day, "
                     "week_1, week_2, week_3, week_4")
        if user['role'] == 'manager':
            rows = db.execute(
                f"SELECT {cust_cols} FROM customers WHERE name LIKE ? ORDER BY name LIMIT 12",
                (f'%{q}%',)
            ).fetchall()
        else:
            regions = [r.strip() for r in user['regions'].split(',') if r.strip()]
            ...
            placeholders = ','.join(['?' for _ in regions])
            sql_search = (
                f"SELECT {cust_cols} FROM customers WHERE name LIKE ? AND region IN (" + placeholders + ") ORDER BY name LIMIT 12"
            )
            rows = db.execute(
                sql_search,
                [f'%{q}%'] + regions
        ...
        # מיפוי אזור → סוכן (שם + אובייקט מלא לחישוב מחזור השבוע)
        agents = db.execute("SELECT name, username, regions FROM users WHERE role='agent'").fetchall()
        region_to_agent = {}
        region_to_agent_obj = {}
        for a in agents:
            for r in (a['regions'] or '').split(','):
                key = r.strip()
                region_to_agent[key] = a['name']
                region_to_agent_obj[key] = {'name': a['name'], 'username': a['username'] or ''}

        result = []
        for r in rows:
            heb = (r["assigned_visit_day"] or r["visit_day"] or "").strip()
            agent_obj = region_to_agent_obj.get(r["region"] or "")
            week_flags = {
                1: r["week_1"] or 0, 2: r["week_2"] or 0,
                3: r["week_3"] or 0, 4: r["week_4"] or 0,
            }
            dates = compute_next_visit_dates(heb, week_flags, agent_obj, 6)
            result.append({
                "id": r["id"], "name": r["name"],
...
@app.get("/api/lost-customers")
async def api_lost_customers(request: Request, refresh: int = 0):
    """לקוחות אבודים למנהל — מקובצים לפי סוכן, עם מונה weeks_missed.
    ברירת מחדל: קורא את הצילום השבועי העדכני. refresh=1: חישוב live לשבוע הנוכחי (בלי לשמור)."""
    import traceback
    user = get_current_user(request)
    if not user or user['role'] != 'manager':
        return JSONResponse({"error": "אין הרשאה"}, status_code=403)
    db = get_db()
    try:
        latest = db.execute("SELECT MAX(week_ending) AS w FROM lost_customer_snapshots").fetchone()
        latest_week = latest['w'] if latest else None

        if refresh or not latest_week:
            # חישוב חי לשבוע הנוכחי — לא נשמר, לא שולח התראה
            lost_map, week_end_s, counted_through = _compute_lost_this_week(db, now_il().date())
            prev_map = _prev_weeks_missed_map(db, week_end_s)
            agents_out = _lost_output_from_map(lost_map, prev_map)
            live = True
        else:
            week_end_s = latest_week
            counted_through = week_end_s  # צילום שמור = כל השבוע (עד חמישי)
            rows = db.execute(
                """SELECT s.customer_id, s.agent_id, s.weeks_missed, s.due_date,
                          c.name, c.city, c.region, u.name AS agent_name
                   FROM lost_customer_snapshots s
                   JOIN customers c ON c.id = s.customer_id
                   LEFT JOIN users u ON u.id = s.agent_id
                   WHERE s.week_ending = ?
                   ORDER BY u.name, s.weeks_missed DESC, c.name""",
                (week_end_s,)
            ).fetchall()
            grouped = {}
            for r in rows:
                ...
                if key not in grouped:
                    grouped[key] = {'agent_id': r['agent_id'],
                                    'agent_name': r['agent_name'] or '—', 'customers': []}
                grouped[key]['customers'].append({
                    'id': r['customer_id'], 'name': r['name'],
                    'city': r['city'] or '', 'region': r['region'] or '',
                    'weeks_missed': r['weeks_missed'], 'due_date': r['due_date'],
                })
            agents_out = sorted(grouped.values(), key=lambda a: a['agent_name'])
            live = False

        total = sum(len(a['customers']) for a in agents_out)
        return JSONResponse({
            "week_ending": week_end_s, "counted_through": counted_through,
            "agents": agents_out, "total": total, "live": live,
        })
    except Exception:
        err = traceback.format_exc()
        print(f"[LOST CUSTOMERS ERROR]\n{err}")
        return JSONResponse({"error": "שגיאה בטעינת לקוחות אבודים", "detail": err}, status_code=500)
    finally:
        db.close()
...
@app.get("/api/unplanned-history")
async def unplanned_history(request: Request):
    user = get_current_user(request)
    if not user or user['role'] != 'agent':
        return JSONResponse({"error": "לא מורשה"}, status_code=403)
    db = None
    try:
        db = get_db()
        rows = db.execute(
            """SELECT v.id, v.visit_date, v.notes, c.name as store_name, c.city
               FROM visits v JOIN customers c ON v.customer_id=c.id
               WHERE v.user_id=? AND v.is_unplanned=1
               ORDER BY v.visit_date DESC, v.id DESC
               LIMIT 30""",
            (user['id'],)
        ).fetchall()
        return JSONResponse({"visits": [dict(r) for r in rows]})
    except Exception as e:
        return JSONResponse({"visits": [], "error": str(e)})
    finally:
        if db:
            try:
                db.close()
...
async def manager_agent_view(request: Request, agent_id: int, date: str = None, status: str = ""):
    ...
    try:
        unplanned_rows = db.execute(
            """SELECT v.id, v.notes, c.name as store_name, c.city
               FROM visits v JOIN customers c ON v.customer_id=c.id
               WHERE v.user_id=? AND v.visit_date=? AND v.is_unplanned=1
               ORDER BY v.id DESC""",
            (agent_id, str(selected_date))
        ).fetchall()
        unplanned_visits = [dict(r) for r in unplanned_rows]
    except Exception:
        unplanned_visits = []
...
async def manager_dashboard_v2(request: Request, date: str = None, agent_id: int = None):
    ...
    # מסך המנהל אוחד למסך יחיד — /v2/manager מפנה ל-/manager (מניעת כפילות ובלבול)
    from urllib.parse import urlencode
    _qs = {}
    if date:
        _qs['date'] = date
    if agent_id:
        _qs['agent_id'] = agent_id
    return RedirectResponse("/manager" + ("?" + urlencode(_qs) if _qs else ""))
...
async def api_analyze(
    ...
    import urllib.error  # מבטיח ש-_urllib.HTTPError זמין ב-except גם אם השגיאה לפני הבקשה
    user = get_current_user(request)
    if not user or user['role'] != 'manager':
...
""")
    ...
    except _urllib.HTTPError as http_err:
        # לוכד את גוף התשובה של Anthropic כדי לחשוף את סיבת ה-400/401 המדויקת
        body = ""
        try:
            body = http_err.read().decode("utf-8", "replace")
        except Exception:
            pass
        detail = f"Anthropic API HTTP {http_err.code}: {body}"
        print(f"[ANALYZE ERROR] {detail}")
        return JSONResponse({"error": "שגיאה בניתוח", "detail": detail}, status_code=500)
    except Exception:
        err = traceback.format_exc()
