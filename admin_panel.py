...
async def admin_reports_page(request: Request):
    ...
    rows = db.execute("""
        ...
        WHERE u.role = 'agent'
        GROUP BY u.id, u.name, c.id, c.name, c.city
        ORDER BY u.name ASC, visit_count DESC
    """).fetchall()
    ...
    for r in rows:
        ...
        if aid not in agents:
            agents[aid] = {"name": r["agent_name"], "customers": []}
        agents[aid]["customers"].append({
            "name":       r["customer_name"],
            "city":       r["city"] or "",
            "visit_count": r["visit_count"],
            "last_visit":  r["last_visit_date"] or "—",
        })

    return _tpl.TemplateResponse("admin_reports.html", {
        "request": request,
        "cfg":     COMPANY_CONFIG,
        "agents":  agents,
    })
