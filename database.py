...
def _ensure_pool():
    ...
    with _pg_pool_lock:
        if _pg_pool is None:
            ...
            # lock_timeout=10s ברמת החיבור: אם ALTER ב-startup ממתין לנעילה
            # שמוחזקת ע"י חיבור תקוע — הוא נכשל אחרי 10ש' (ונתפס ב-try/except)
            # במקום להקפיא את הפריסה לנצח (הלקח מתקלת ה-locks 2026-07-13).
            _pg_pool = psycopg2.pool.ThreadedConnectionPool(
                2, 10, url, options='-c lock_timeout=10000'
            )
    return _pg_pool
...
def init_db():
    ...
    if _is_postgres():
        ...
        stmts = [
            ...
            """ALTER TABLE visits ADD COLUMN IF NOT EXISTS is_unplanned INTEGER DEFAULT 0""",
            """CREATE TABLE IF NOT EXISTS lost_customer_snapshots (
                id SERIAL PRIMARY KEY,
            ...
            """ALTER TABLE lost_customer_snapshots ADD COLUMN IF NOT EXISTS due_date TEXT""",
        ]
        for stmt in stmts:
            try:
                db.execute(stmt)
        ...
        for _idx in [
            ...
            "CREATE INDEX IF NOT EXISTS idx_lost_week      ON lost_customer_snapshots(week_ending)",
            "CREATE INDEX IF NOT EXISTS idx_lost_cust_week ON lost_customer_snapshots(customer_id, week_ending)",
        ]:
            try:
                db.execute(_idx)
            except Exception:
        ...
        db.execute('''CREATE TABLE IF NOT EXISTS lost_customer_snapshots (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            week_ending TEXT NOT NULL,
            customer_id INTEGER NOT NULL,
            agent_id INTEGER,
        ...
        try:
            db.execute("ALTER TABLE lost_customer_snapshots ADD COLUMN due_date TEXT")
        except Exception:
            pass
        ...
        try:
            ...
            pass
        # SQLite migrations — visits (unplanned visit)
        try:
            db.execute('ALTER TABLE visits ADD COLUMN is_unplanned INTEGER DEFAULT 0')
        except Exception:
            pass
